        ...
        <div className="max-w-5xl mx-auto space-y-6 sm:space-y-8">
          {EXPERIENCES.map((exp, index) => (
            <ScrollReveal key={exp.id} delay={index * 120}>
              <div className="w-full rounded-2xl bg-[#0a0a0c] border border-white/10 hover:border-[#cfa856]/70 p-6 sm:p-8 transition-all duration-300 shadow-2xl hover:shadow-[#cfa856]/10 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-b border-white/10 pb-4">
                  ...
                  <div className="space-y-1">
                    <div className="flex flex-wrap items-center gap-3">
                      <h3 className="text-xl sm:text-2xl font-black font-sans text-white uppercase tracking-wide">
                        {exp.role}
                      </h3>

                      {exp.isCurrent && (
                        <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full bg-emerald-500/20 border border-emerald-500/50 text-emerald-400 text-[10px] font-mono font-bold tracking-widest uppercase">
                    ...
                    <div className="text-xs sm:text-sm font-mono text-[#cfa856] font-semibold flex items-center gap-2 pt-0.5">
                      <Building2 className="w-4 h-4 text-[#cfa856]" />
                  ...
                  {/* Period Badge */}
                  <div className="self-start sm:self-center inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full bg-[#cfa856]/10 border border-[#cfa856]/40 text-[#cfa856] text-xs font-mono font-bold uppercase tracking-wider shrink-0">
                ...
                <div className="space-y-3 pt-1">
                  {exp.highlights.map((point, idx) => (
                    <div key={idx} className="flex items-start gap-3 text-xs sm:text-sm text-slate-200 leading-relaxed">
                      <CheckCircle2 className="w-4 h-4 text-[#cfa856] shrink-0 mt-0.5" />
